        ...
        <div class="d-flex align-items-center justify-content-between gap-4">
            ...
            <div class="d-flex align-items-center gap-3 flex-shrink-0 ms-2">
                <div class="d-flex flex-column d-none d-sm-flex">
                    <h5 class="mb-0 fw-bold text-white lh-1">@yield('title', 'Dashboard')</h5>
                </div>
            </div>

            <!-- Center Column (Spacer or Secondary Info) -->
                                            ...
                                            <div class="flex-grow-1">
                                                ...
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <small class="text-muted" style="font-size: 0.7rem;">
                                                        <i class="far fa-clock me-1"></i>{{ isset($notification->created_at) ? $notification->created_at->diffForHumans() : '' }}
                                                    </small>
                                                    @if(($notification->Status ?? '') === 'Unread')
                                                        <form action="{{ route('notifications.markAsRead', $notification->id ?? 0) }}" method="POST">
                                                            @csrf
                    ...
                    <ul class="dropdown-menu dropdown-menu-end border-0 shadow-lg" aria-labelledby="profileDropdown" style="border-radius: 16px; margin-top: 12px; min-width: 240px; animation: slideDown 0.3s ease;">
                        <li class="px-3 py-2 border-bottom" style="background: linear-gradient(135deg, #f8f9fa 0%, #ffffff 100%); border-radius: 16px 16px 0 0;">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                @endif
                                <div>
                                    <h6 class="mb-0 fw-bold text-dark">{{ Auth::user()->FirstName ?? (Auth::user()->name ?? 'User') }} {{ Auth::user()->LastName ?? '' }}</h6>
                                    <small class="text-muted">{{ Auth::user()->email ?? '' }}</small>
                                </div>
                            </div>
                        </li>
                        <li>
                            <a class="dropdown-item py-2 d-flex align-items-center dropdown-link-custom" href="#" data-bs-toggle="offcanvas" data-bs-target="#profileOffcanvas" aria-controls="profileOffcanvas">
                                <i class="fas fa-user me-3 opacity-75" style="width: 20px;"></i> <span>Profile</span>
                            </a>
                        </li>
                        <li>
                            <a class="dropdown-item py-2 d-flex align-items-center dropdown-link-custom" href="{{ route('notifications') }}">
                                <i class="fas fa-bell me-3 opacity-75" style="width: 20px;"></i> <span>Notifications</span>
                            </a>
                        </li>
...
<style>
    ...
    body:not(.dark-mode) header .sidebar-branding-text {
        color: #2C3E50 !important;
    }

    body:not(.dark-mode) .opacity-75 {
    ...
    body:not(.dark-mode) .profile-photo-navbar {
    ...
    body.dark-mode .dropdown-link-custom {
        color: rgba(255,255,255,0.9) !important;
    }

    body.dark-mode .dropdown-link-custom:hover {
