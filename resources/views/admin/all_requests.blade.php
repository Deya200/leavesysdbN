...
@section('content')
    <div class="container-fluid py-4">
        ...
        <div class="alert alert-info alert-dismissible fade show" role="alert">
            ...
            <strong>System Overview:</strong> View all leave requests submitted by employees. Use filters to search by
            employee, leave type, or status.
            ...
            <div class="col-md-3 mb-2">
                <div class="stats-card" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);">
                    <div class="stats-number">
                        {{ $allRequests->whereIn('RequestStatus', ['Pending Supervisor Approval', 'Pending Admin Verification'])->count() }}
                    </div>
                ...
                <form method="GET" action="{{ route('leave_requests.admin_all') }}" class="row g-3">
                    <div class="col-md-5">
                        <input type="text" name="search" class="form-control search-box"
                            placeholder="Search by employee name, number, or leave type..." value="{{ $search }}">
                    </div>
                    <div class="col-md-4">
                        <select name="status" class="form-select search-box">
                            <option value="">All Status</option>
                            @foreach($statuses as $status)
                                <option value="{{ $status }}" {{ $status === request('status') ? 'selected' : '' }}>{{ $status }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="small fw-bold">
                                            {{ \Carbon\Carbon::parse($request->StartDate)->format('M d, Y') }}</div>
                                        <div class="text-muted small">to
                                            {{ \Carbon\Carbon::parse($request->EndDate)->format('M d, Y') }}</div>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center pe-4">
                                        <a href="{{ route('leave_requests.show', $request->LeaveRequestID) }}"
                                            class="btn btn-sm btn-outline-primary rounded-pill" title="View request details">
