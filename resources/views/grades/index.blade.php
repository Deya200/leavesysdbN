...
<style>
    ...
    .table thead th:nth-child(2) {  /* Grade Name column */
        text-align: left;
        padding-left: 20px;
    }
    
    .table thead th:nth-child(3) {  /* Annual Leave Days column */
        text-align: center;
    }
    ...
    .table td:nth-child(2) {  /* Grade Name column */
        text-align: left;
        padding-left: 20px;
    }
    
    .table td:nth-child(3) {  /* Annual Leave Days column */
        text-align: center;
    }
...
<div class="grades-container mt-4">
    ...
    <div class="card-custom mb-4">
        ...
        <div class="header-card">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center">
                ...
                <div class="d-flex align-items-center mb-3 mb-md-0">
                    <i class="fas fa-chart-line fa-lg me-3" style="color: #2E3A87;"></i>
                    <h5 class="mb-0" style="font-weight: 600; color: #2E3A87;">Grade Management</h5>
                </div>
                
                <!-- Welcome Message -->
                <div class="text-md-end">
                    <small class="text-muted d-block">Welcome, {{ auth()->user()->FirstName ?? 'Admin' }}!</small>
                    <small class="text-muted">Manage job grades and annual leave allowances</small>
                </div>
            </div>
        </div>
        ...
        <!-- Search Form -->
        <div class="mx-3 mt-3">
            <form action="{{ route('grades.index') }}" method="GET" class="search-form">
        ...
        <div class="table-card">
            @if ($grades->isNotEmpty())
                                ...
                                <tr class="{{ $index >= 10 ? 'hidden-row' : '' }}">
                                    ...
                                    <td>
                                        <div class="d-flex gap-2 justify-content-center">
                                            <a href="{{ route('grades.edit', $grade->GradeID) }}" 
                                               class="btn btn-edit" 
                                               title="Edit Grade">
                                               <i class="fas fa-edit me-1"></i>
                                               Edit
                                            </a>
                                            <form action="{{ route('grades.destroy', $grade->GradeID) }}" 
                                                  method="POST" 
                ...
                <div class="text-center see-more-btn">
                    <button id="showMoreBtn" class="btn btn-outline-primary btn-sm">
                        <i class="fas fa-chevron-down me-1"></i>
                        See More
                    </button>
                    <button id="showLessBtn" class="btn btn-outline-secondary btn-sm" style="display: none;">
            ...
            @else
                <div class="text-center py-5">
                    ...
                    <h5 class="text-muted mb-3">No grades found</h5>
                    <p class="text-muted mb-4">
                        @if($search)
                            No results found for "{{ $search }}"
                        @else
                            Start by adding your first grade to the system
                        @endif
                    </p>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                const gradeName = this.closest('tr').querySelector('.grade-badge').textContent;
