...
@section('title', 'Create Locum Rate')
        ...
        <div class="col-md-8 offset-md-2">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Create New Locum Rate</h3>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.locum_rates.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="position_type" id="position_type" class="form-control @error('position_type') is-invalid @enderror"
                                   value="{{ old('position_type') }}" placeholder="e.g., Nurse, Doctor, Specialist" required>
                            @error('position_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="shift" id="shift" class="form-control @error('shift') is-invalid @enderror" required>
                                ...
                                <option value="day" {{ old('shift') == 'day' ? 'selected' : '' }}>Day Shift (7:30 AM - 4:30 PM)</option>
                                <option value="night" {{ old('shift') == 'night' ? 'selected' : '' }}>Night Shift (4:30 PM - 8:30 AM)</option>
                            </select>
                            @error('shift')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="daily_rate" id="daily_rate" class="form-control @error('daily_rate') is-invalid @enderror"
                                   value="{{ old('daily_rate') }}" step="0.01" min="0" required>
                            @error('daily_rate')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> Create Rate
                            </button>
                        </div>
                    </form>
                </div>
